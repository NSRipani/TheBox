...
def autoCompletado(self,QDate,mensaje_ingreso_datos):
    ...
    horas_h = self.tablaHoras.item(fila,4).text()
    fecha_h = self.tablaHoras.item(fila, 5).text()
...
def tabla_HorasXEmpleado(self,cursor,busqueda,QHeaderView,QTableWidget,QAbstractItemView,QTableWidgetItem,Qt,QDate):
    ...
    for i, row in enumerate(busqueda):
        for j, val in enumerate(row):
            ...
            if j == 4:  # Ajustar alineación para ciertas columnas
                item.setTextAlignment(Qt.AlignmentFlag.AlignRight)
            if j in [0,5]:  # Ajustar alineación para ciertas columnas
                item.setTextAlignment(Qt.AlignmentFlag.AlignCenter) 
            #Indices de las columnas que contienen fechas
            if j == 5:  
                fecha = QDate.fromString(str(val), "yyyy-MM-dd")  # Convertir la fecha a objeto QDate
    ...
    # Calcular y mostrar la suma de las horas diarias en la fila adicional
    total_horas = sum(int(row[4]) for row in busqueda)
    ...
    total_horas_empleados.setTextAlignment(Qt.AlignmentFlag.AlignRight)
    self.tablaHoras.setItem(len(busqueda), 3, total_horas_empleados)
    ...
    item_seuma_horas.setTextAlignment(Qt.AlignmentFlag.AlignRight)
    self.tablaHoras.setItem(len(busqueda), 4, item_seuma_horas)
...
def tabla_HorasTotales(self,cursor,busqueda,QHeaderView,QTableWidget,QAbstractItemView,QTableWidgetItem,Qt,QDate):
    ...
    for i, row in enumerate(busqueda):
        ...
        item_total_horas2.setTextAlignment(Qt.AlignmentFlag.AlignRight)
        self.tablaHoras.setItem(len(busqueda), 3, item_total_horas2)
        ...
        self.tablaHoras.setItem(len(busqueda), 4, item_suma_horas2)
